...
source("fit_submissions_gam.R", chdir=TRUE)
...
fit_and_bootstrap <- function(
  ...
  print("Raw Result")
  raw_result <- fit_data(raw_data, params, election_config, method=method)

  # bs_list <- lapply(as.list(1:n_boot), bootstrap_once)
  
  if(USE_PARALLEL){
    bs_list <- foreach(seed=1:n_boot) %do% 
      bootstrap_once(seed, raw_data, raw_result, params, config, method)
  } else {
    bs_list <- vector(mode="list", length=n_boot)
    for(seed in 1:n_boot){
      bs_list[[seed]] <- bootstrap_once(seed, raw_data, raw_result, params, config, method)
    }
  }
...
fit_data <- function(data, params, config, verbose=TRUE, method="gam"){
  if(method=="loess"){
    ...
    single_result <- process_results_gam(
      single_fit,
      election_config=config,
      plots = FALSE,
      save_results = FALSE,
...
bootstrap_once <- function(seed, raw_data, raw_result, params, config, method="gam"){
  print(sprintf("Bootstrap %s", seed))
  set.seed(seed)
  bs_data <- raw_data %>% sample_frac(replace=TRUE) 
  bs_result <- fit_data(
    bs_data, params, config, method=method
  )
