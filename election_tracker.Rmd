...
precinct_turnout <- bs$single_result$full_predictions %>%
  filter(time_of_day == current_time) %>%
  group_by(precinct) %>%
  summarise(
    turnout = sum(prediction),
...
write.csv(
  precinct_turnout, 
  file=sprintf("outputs/precinct_turnout_%s.csv", config$city_filename), 
  row.names=FALSE
)
...
turnout_plot_file <- sprintf(
  "outputs/turnout_plot_%s.png",
  format(Sys.time(), "%H_%M_%S")
)
ggsave(p, file = turnout_plot_file)

p
```

As of `r strip_leading_zero(format(current_time, "%I:%M"))`, `r scales::comma(nrow(raw_data))` voters in `r length(unique(raw_data$ward))` wards have [shared their voter number](`r submission_bitly`). I estimate that <b>`r scales::comma(round(turnout_ci$turnout_500))`</b> people have voted across the city (95% Confidence Interval: `r scales::comma(round(turnout_ci$turnout_025))` - `r scales::comma(round(turnout_ci$turnout_975))`). `r mail_in_text`
...
p <- gg_wards_relative(wards, current_time, config)
relative_map_file <- sprintf(
  "outputs/relative_map_%s.png",
  format(Sys.time(), "%H_%M_%S")
)
ggsave(p, file = relative_map_file)
p
...
stargazer(
  ward_turnout %>% 
    left_join(ward_ci) %>%
    mutate(time_of_day=strip_leading_zero(format(current_time, "%I:%M"))) %>%
...
stargazer(
  raw_data %>% 
    mutate(
      time = strip_leading_zero(
        format(config$base_time + minutes(minute), "%I:%M")
      )
    ) %>%
